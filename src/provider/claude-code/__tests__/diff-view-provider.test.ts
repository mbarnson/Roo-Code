...
// Mock i18next
jest.mock("i18next", () => ({
	t: jest.fn((key, text, params) => {
		if (text === "Apply Changes") return "Apply Changes"
...
describe("DiffViewProvider", () => {
	...
	describe("showDiff", () => {
		it("should create temporary files and show a diff", async () => {
			const result = await provider.showDiff("/test/file.js", "original content", "modified content")
		...
		it("should handle errors gracefully", async () => {
			...
			// Also mock the file creation to ensure we get to the error handling
		...
		it("should create the temp directory if it does not exist", async () => {
			...
			await provider.showDiff("/test/file.js", "original content", "modified content")
		...
		it("should handle cleanup errors gracefully", async () => {
			// Reset mock behavior to ensure consistent test
			...
			// Verify user selection still works correctly
			expect(result).toBe(true)
		})
	})
