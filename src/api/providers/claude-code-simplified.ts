...
export class ClaudeCodeHandler extends BaseProvider implements ApiHandler, SingleCompletionHandler {
	 ...
	 *
	 * Uses tiktoken when available, or falls back to character-based estimation.
	 *
 ...
 *
 * Combines models from centralized definitions and the CLI.
 * Falls back to default models if CLI is unavailable or user is not authenticated.
 *
 * @param claudeCodePath - Path to the Claude Code CLI executable (defaults to "claude-code")
